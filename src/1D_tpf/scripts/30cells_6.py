...
from hc import DiffusionHC, FluxHC4, FluxHC5  # noqa: E402; solve,
from model import TPFModel  # noqa: E402
from plotting import solve_and_plot  # noqa: E402

results_dir = pathlib.Path(__file__).parent / "30cells_6_results"
...
model_params = {
    ...
    "pe": jnp.concatenate(
        [
            jnp.full((NUM_CELLS // 2,), 0.05),
            jnp.full((NUM_CELLS // 2,), 100.0),
        ]
    ),
    ...
    "initial_conditions": jnp.stack(
        [
            jnp.zeros((NUM_CELLS,)),
            jnp.concatenate(
                [jnp.full((NUM_CELLS // 2,), 0.98), jnp.full((NUM_CELLS // 2,), 0.02)]
            ),
        ],
        axis=1,
    ).flatten(),
