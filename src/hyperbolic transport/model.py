...
def corey(s, mobility_ratio):
    return s**2 / (s**2 + mobility_ratio * (1 - s) ** 2)


def corey_prime(s, mobility_ratio):
    return (2 * s * (1 - s) ** 2) / (s**2 + mobility_ratio * (1 - s) ** 2) ** 2


def corey_double_prime(s, mobility_ratio):
    return (2 * (1 - s) ** 2 * (mobility_ratio * (1 - s) ** 2 - s**2)) / (
        s**2 + mobility_ratio * (1 - s) ** 2
    ) ** 3


def compute_adaptive_diffusion_coefficient(f_prime, mobility_ratio, delta_t, omega):
    s_val = np.linspace(0, 1, 100)
    max_wave_speed = np.abs(f_prime(s_val, mobility_ratio)).max()
    return omega * delta_t * max_wave_speed


def g_diffusion(f, s, mobility_ratio, adaptive_diffusion_coefficient, prev_s):
    return f(s, mobility_ratio) + adaptive_diffusion_coefficient * (s - prev_s)


def g_diffusion_prime(s, mobility_ratio, adaptive_diffusion_coefficient):
    return f_prime(s, mobility_ratio) + adaptive_diffusion_coefficient


def g_diffusion_double_prime(s, mobility_ratio):
    return f_double_prime(s, mobility_ratio)

def create_homotopy(f, g, f_prime, g_prime, f_double_prime, g_double_prime)
    def h(beta, **kwargs):
